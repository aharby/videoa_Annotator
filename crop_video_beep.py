...
class getDelayOfBeep:    
    
    def get_cropped_file(self, filtered_beep, filtered_facecam):
...
reference_sound = input("Please type in directory of reference beep file (including . extension)\n")
facecam_video = input("Please type in directory of face_cam video to crop (including . extension)\n")

filtered_beep = convertToWaveFile()
filtered_facecam = convertToWaveFile()

a=filtered_beep.convert_to_wavFile_and_filter(reference_sound)
b=filtered_facecam.convert_to_wavFile_and_filter(facecam_video)

cropped_facecam = getDelayOfBeep()
cropped_facecam.get_cropped_file(a, b) #a new file is created with everything before the beep sound cropped out
